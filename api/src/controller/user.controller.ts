...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      const errorMessage =
        error instanceof AppError ? error.message : 'User login failed!';
...
router.get('', async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!isAdmin) {
    return next(
      new AppError('Admin privileges are needed to access this route!', 403)
    );
  }
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      await userService.changeUserPassword(req as AuthRequest);
    } catch (error) {
      const errorMessage =
        error instanceof AppError ? error.message : 'User login failed!';
      const errorCode = error instanceof AppError ? error.code : 500;
