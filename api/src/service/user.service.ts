...
export const loginUser = async (userLoginRequest: UserLoginRequest): Promise<UserLoginResponse> => { 
  
  const existingUser = await UserRepository.findOneBy({ email: userLoginRequest.email});
  if(!existingUser){
    throw new AppError("User with provided email does not exist!", 404);
    
  }

  const isPasswordCorrect = await argon2.verify(existingUser.password, userLoginRequest.password);
  ...
  return UserLoginResponse.toDto(token, existingUser.role === UserRole.ADMIN, existingUser.id);
